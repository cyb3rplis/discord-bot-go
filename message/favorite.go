...
func HandleFavorite(s *discordgo.Session, m *discordgo.MessageCreate, arg, arg2, command string) error {
	...
	case "add":
		...
		if soundID != "" {
			// Send message to bot
			utils.NewMessageRoutine(command+"help", fmt.Sprintf("🧐 Sound: '%s' already exists in your favorites\n", arg2), s, m, false)
			return nil
		}
		...
		if err != nil {
			// Send message to bot
			utils.NewMessageRoutine(command+"help", fmt.Sprintf("🧐 Error adding sound: '%s'. Check if sound really exists\n", arg2), s, m, false)
			return err
		}
		message := fmt.Sprintf("🔥  Sound added to favorites: %s\n", arg2)
		utils.NewMessageRoutine(command, message, s, m, false)
		return nil
	case "remove":
		// remove sound from favorites
		err := SoundFavoriteRemove(m, arg2)
		if err != nil {
			utils.NewMessageRoutine(command+"help", fmt.Sprintf("🧐 Error removing sound: '%s'. Check if sound really exists\n ", arg2), s, m, false)
			return err
		}
