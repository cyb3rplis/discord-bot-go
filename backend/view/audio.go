	...
	if i.Type == discordgo.InteractionApplicationCommand {
		...
		case "audio":
			...
			case "last":
				...
				if err != nil {
					log.ErrorLog.Println("error executing audio command:", err)
				}

				download := Download{SoundName: a.model.Config.AudioTemp}
				err = a.PlayAudio(download.SoundName, s, i)
				if err != nil {
					log.ErrorLog.Println("error playing audio:", err)
				}
			...
			case "play":
				...
				if err != nil && err != sql.ErrNoRows {
					...
					if user, ok := SetUserGulagRemaining(user); ok {
						...
						if err != nil {
							log.ErrorLog.Printf("error[audio1] sending message: %v", err)
						}
						return
					}
				}
				...
				if err != nil {
					log.ErrorLog.Println("error[audio2] sending message:", err)
				}
				...
				if err != nil {
					log.ErrorLog.Println("error[audio3] sending message:", err)
				}
			...
			default:
				...
				if err != nil {
					log.ErrorLog.Println("fallback to default audio handler", err)
				}
			}
		}
	}
