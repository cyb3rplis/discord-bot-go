...
func HandleTTS(s *discordgo.Session, m *discordgo.MessageCreate, command string) error {
	...
	if m.Content == fmt.Sprintf("%stts", prefix) {
		...
		NewMessageRoutine(command+"help", msg, s, m)
	...
	user, err := model.GetUserFromUsername(m.Message.Author.GlobalName)
	if err != nil {
		...
		if remaining, ok := IsUserInGulag(user); ok {
			...
			NewMessageRoutine(".gulag"+user.ID, msg, s, &discordgo.MessageCreate{Message: m.Message})
	...
	if strings.HasPrefix(ttsText, "\"") && strings.HasSuffix(ttsText, "\"") {
		...
		if re.MatchString(ttsText) {
			err := VoiceChannelCheck(s, m)
			if err != nil {
				logger.ErrorLog.Println("error checking voice channel:", err)
				return err
			}

			model.CleanUpSoundFile("tts")

			err = textToSpeech(ttsText)
			if err != nil {
	...
	NewMessageRoutine(command+"quote", msg, s, m)
	return nil
}
