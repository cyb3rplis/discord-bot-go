...
func LoadSound(soundName string) error {
	...
	err := model.Bot.Db.QueryRow("SELECT file FROM sounds WHERE name = ?", soundName).Scan(&fileData)
	if err != nil {
...
func SyncDatabaseWithFileSystem(folderMap map[string][]string) error {
	existingCategories := getCategories() // Get current folders/categories in DB
	...
	for folder, files := range folderMap {
		...
		if dbCategoryID, exists := existingCategories[folder]; exists {
			...
			categoryID = getCategoryByID(folder)
		}
...
func addCategory(folderName string) error {
	...
	var existingID int
	err := model.Bot.Db.QueryRow("SELECT id FROM categories WHERE name = ?", folderName).Scan(&existingID)
...
func addSound(categoryID int, fileName, fileHash string, fileData []byte) error {
	...
	var existingID int
	err := model.Bot.Db.QueryRow("SELECT id FROM sounds WHERE hash = ?", fileHash).Scan(&existingID)
	if err != nil && err != sql.ErrNoRows {
...
func GetSoundIDByName(name string) (soundName string, err error) {
	err = model.Bot.Db.QueryRow("SELECT id FROM sounds WHERE name = ?", name).Scan(&soundName)
	if err != nil {
...
func GetFavoriteByNameAndUserID(name, userID string) (soundName string, err error) {
	err = model.Bot.Db.QueryRow("SELECT user_favorites.id FROM user_favorites LEFT JOIN sounds s on user_favorites.sound_id = s.id WHERE name = ? AND user_id = ?", name, userID).Scan(&soundName)
	if err != nil {
